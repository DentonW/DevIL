...
// Last modified:  01/04/2001 <--Y2K Compliant! =]
//
// Filename: testil/windowstest/windowstest.c
//
// Description:  Full GDI test application for OpenIL.
//
//-----------------------------------------------------------------------------
...
void CreateGDI()
{
	...
	hReplaced = (HBITMAP)SelectObject(hMemDC, hBitmap);
	ilBindImage(CurName);
	if (CurImg)
...
void DestroyGDI()
{
	if (hMemDC) {
		...
		DeleteObject(SelectObject(hMemDC, hReplaced));
		DeleteDC(hMemDC);
	}
	hBitmap = NULL;
	hMemDC = NULL;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static char *OFilter[] = {
		...
		"Image Files (All Supported Types)", "*.jpe;*.jpg;*.jpeg;*.bmp;*.ico;*.pbm;*.pgm;*.pnm;*.ppm;*.png;*.bw;*.rgb;*.rgba;*.sgi;*.tga;*.tif;*.tiff;*.pcx",
	...
	switch (message)
	{
		...
		case WM_PAINT:
			...
			BitBlt(hDC, 0, 0, (WORD)BmpInfo.biWidth, (WORD)BmpInfo.biHeight, 
				  hMemDC, 0, 0, SRCCOPY);
			EndPaint(hWnd, &ps);
		...
		case WM_COMMAND:
			...
			switch (LOWORD(wParam))
			{
				...
				case ID_FILE_PRINT:
					return (0L);  // Not tested yet, so just return.
					PRINTDLG	Pd;
					DOCINFO		Di;
					HDC			PrintDC;
					...
					Di.lpszDocName = "OpenIL Printing Test";
					Di.lpszOutput = NULL;
					...
					SelectObject(PrintDC, hBitmap);
					StretchBlt(Pd.hDC, 0, 0, Width, Height, PrintDC, 0, 0, Width, Height, SRCCOPY);
			...
			switch (LOWORD(wParam))
			{
				...
				case ID_FILTER_GAMMACORRECT:
					if (DialogBox(hInstance, MAKEINTRESOURCE(IDD_DIALOG_FILTER),
						hWnd, FilterDlgProc) == TRUE) {
						iluGammaCorrectCurve(FilterParamFloat);
					}
					break;
